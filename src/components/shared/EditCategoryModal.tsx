    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Modifier la catégorie "${category.name}"`}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <Input id="cat-name" value={name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setName(e.target.value)} />
        </div>
        <div>
          <label htmlFor="cat-desc" className="block text-sm font-medium text-foreground">Description</label>
          <textarea id="cat-desc" value={description} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setDescription(e.target.value)} rows={3} className="w-full mt-1 border-border rounded-md shadow-sm focus:ring-ring focus:border-primary" />
        </div>
        <div>
          <label htmlFor="cat-image" className="block text-sm font-medium text-foreground">Image de la catégorie</label>
          <Input id="cat-image" type="file" onChange={(e: React.ChangeEvent<HTMLInputElement>) => setImageFile(e.target.files ? e.target.files[0] : null)} />
          {category.heroImage && (
